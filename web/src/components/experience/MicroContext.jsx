  ...
  useEffect(() => {
    ...
    const rotate = (time) => {
      if (lastFrameTimeRef.current !== null) {
        ...
        setRotation((current) => (current + (delta * 360) / 28000) % 360);
      }
          ...
          <div className="relative mx-auto h-56 w-56">
            ...
            <div className="absolute inset-0">
              {trainedCharacters.map((character, index) => {
                ...
                const positionAngle = angle + rotation;
                return (
                  <button
                    key={character.id}
                    onClick={() => {
                    ...
                    style={{ transform: `translate(-50%, -50%) rotate(${positionAngle}deg) translateY(-88px) rotate(-${positionAngle}deg)` }}
